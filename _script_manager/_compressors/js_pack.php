...
$headstr = (isset($_GET['nocompress']) ? 'Scripts concatenated' : 'Scripts packed') . ' (' . time() . ')';
...
foreach ($paths as $templatename => $arr) {
    ...
    foreach ($arr['src'] as $src) {
        ...
        $str .= ((!$src['compress'] || !empty($_GET['nocompress'])) ? $s : compress($s, $src['no_commenthead']));
    ...
    $o = str_replace(
        array('TEMPLATE', 'BACKEND', 'FRONTEND', 'LANG'),
        array($arr['base'], $backend, $frontend, $_GET['lang']),
        $arr['out']
    );
